# Adapted from https://raw.githubusercontent.com/hans/glove.py/master/test/test_glove.py
...
f = open("sle_corpus.out","r")
lines = f.readlines()
f.close()
lines = "".join(lines) # because we have new line characters in the string
my_corpus = lines.split("\n")

glove.logger.setLevel(logging.ERROR)
vocab = glove.build_vocab(my_corpus)
cooccur = glove.build_cooccur(vocab, my_corpus, window_size=10)
